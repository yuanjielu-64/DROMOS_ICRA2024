...
void GManager::CallbackEventOnDisplay(void)
{
  if (m_gManager)
  {
    ...
    const bool is3D = HasAllFlags(m_gManager->m_menuItems[m_gManager->MENU_DRAW_3D]->GetFlags(), GMenuItem::FLAG_ON);
...
{
  const double *p = HasAllFlags(m_menuItems[MENU_CAMERA_ROTATE_CENTER]->GetFlags(), GMenuItem::FLAG_ON) ? m_gCamera.GetCenter() : m_gCamera.GetEye();
  bool event = false;
