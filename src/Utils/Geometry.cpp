...
bool IntersectSegments2D(const double x1, const double y1, const double x2, const double y2, const double x3, const double y3, const double x4, const double y4)
{
...
bool IsPointInsidePolygon2D(const double p[2], const int n, const double poly[])
{
  ...
  for (i = 0; i < n; i++)
  {
    ...
    if (testr || testl)
      x = (poly[2 * i] * poly[2 * i1 + 1] - poly[2 * i1] * poly[2 * i + 1] + py * (poly[2 * i1] - poly[2 * i])) / (poly[2 * i1 + 1] - poly[2 * i + 1]);
...
bool IsPolygonInsidePolygon2D(const int n1, const double poly1[], const int n2, const double poly2[], const bool checkIntersection)
{
  for (int i = 0; i < n1; ++i)
...
bool IsPolygonConvex2D(const int n, const double poly[])
{
  ...
  for (int i = 1; i < n; i++)
    if (turn != (Turn2D(&poly[2 * i], &poly[2 * ((i + 1) % n)], &poly[2 * ((i + 2) % n)]) <= Constants::EPSILON))
      return false;
  return true;
}
...
void SampleRandomPointInsideConvexPolygon2D(const int n, const double poly[], double p[2], const double min[2], const double max[2])
{
...
void SampleRandomPointInsidePolygon2D(const int n, const double poly[], double p[2], const double min[2], const double max[2])
{
  double umin[2], umax[2];
  if (min == NULL || max == NULL)
    BoundingBoxPolygon2D(n, poly, umin, umax);
...
void FixSkeleton2D(std::vector<double> &pts)
{
  ...
  while (2 * i + 2 < pts.size())
  {
    ...
    for (int j = i + 2; 2 * j + 2 < pts.size() && intersect == false; ++j)
    {
      ...
      if (IntersectSegments2D(pts[2 * a], pts[2 * a + 1], pts[2 * b], pts[2 * b + 1], pts[2 * c], pts[2 * c + 1], pts[2 * d], pts[2 * d + 1], p[0], p[1]))
      {
        intersect = true;
...
bool ShiftSkeleton2D(const int n, const double skeleton[], const double thick, std::vector<double> &poly)
{
  ...
  for (int i = 1; i < n; ++i)
  {
    ...
    if (IntersectLines2D(line1[0], line1[1], line1[2], line1[3], line2[0], line2[1], line2[2], line2[3], x, y, mua, mub) == false)
    {
      x = line1[2];
      y = line1[3];
    }
