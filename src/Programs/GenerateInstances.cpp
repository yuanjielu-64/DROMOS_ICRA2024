...
extern "C" int GenerateInstances(int argc, char **argv) {
    ...
    cs.m_shapeMinDims[0] = 2.0;
    cs.m_shapeMinDims[1] = 2.0;
    cs.m_shapeMaxDims[0] = 2.0 + Constants::EPSILON;
    cs.m_shapeMaxDims[1] = 2.0 + Constants::EPSILON;
    ...
    if (data && data->m_params) {
        ...
        cs.SetSeparation(GROUP_OBSTACLES, GROUP_OBSTACLES,
                         data->m_params->GetValueAsDouble(Constants::KW_SeparationObstacleObstacle,
                                                          Constants::SEPARATION_OBSTACLE_OBSTACLE));
        cs.SetSeparation(GROUP_OBSTACLES, GROUP_INITIAL,
                         data->m_params->GetValueAsDouble(Constants::KW_SeparationObstacleInitial,
                                                          Constants::SEPARATION_OBSTACLE_INITIAL));
        cs.SetSeparation(GROUP_OBSTACLES, GROUP_GOALS,
                         data->m_params->GetValueAsDouble(Constants::KW_SeparationObstacleGoal,
                                                          Constants::SEPARATION_OBSTACLE_GOAL));
        cs.SetSeparation(GROUP_INITIAL, GROUP_INITIAL,
                         data->m_params->GetValueAsDouble(Constants::KW_SeparationInitialInitial,
                                                          Constants::SEPARATION_INITIAL_INITIAL));
        cs.SetSeparation(GROUP_INITIAL, GROUP_GOALS,
                         data->m_params->GetValueAsDouble(Constants::KW_SeparationInitialGoal,
                                                          Constants::SEPARATION_INITIAL_GOAL));
        cs.SetSeparation(GROUP_GOALS, GROUP_GOALS, data->m_params->GetValueAsDouble(Constants::KW_SeparationGoalGoal,
                                                                                    Constants::SEPARATION_GOAL_GOAL));

        nrGrids = data->m_params->GetValueAsInt(Constants::KW_NrGrids, nrGrids);
    ...
    nrGrids = params.GetValueAsInt("GenerateInstancesNrGrids", nrGrids);

    auto goalPoints = params.GetValue("GoalPoints", NULL);
    std::vector<double> goalPositions;

    std::ifstream inputFile(goalPoints);
    ...
    while (std::getline(inputFile, line)) {
        ...
        for (const auto &xx: record){
            double position = std::stod(xx);
            goalPositions.push_back(position);
        }
    }

    int goalIdx = RandomUniformInteger(0, nrGrids * nrGrids - 1);
    double x = goalPositions[2 * goalIdx];
    double y = goalPositions[2 * goalIdx + 1];
    goalPositions.erase(goalPositions.begin() + 2 * goalIdx);
    goalPositions.erase(goalPositions.begin() + 2 * goalIdx);

    inputFile.close();

    // generate initial placement
    Logger::m_out << "..generating initial placement" << std::endl;
    int count = 0;
    double T[3];
    ...
    do {
        ...
        if ((++count) % 1 == 0)
            cs.SetSeparation(GROUP_INITIAL, GROUP_OBSTACLES, cs.GetSeparation(GROUP_INITIAL, GROUP_OBSTACLES) * 0.95);
    ...
    while (ggoals->m_objects.size() < (nrGrids * nrGrids - 1) && ggoals->m_boxes3D.size() < 6 * (nrGrids * nrGrids - 1)) {
        double x2 = goalPositions[n];
        double y2 = goalPositions[n + 1];
        count = 0;
        while (cs.AddObject(GROUP_GOALS, 5, rminInitGoal,  rmaxInitGoal, T, x2, y2) == false){
    ...
    for (int i = 0; i < (int) times.size(); ++i) {
        ...
        if (cs.GetGrid()->GetNrDims() == 2) {
            ...
            auto vertices = dynamic_cast<RegionPolygon2D *>(setup.GetProblem()->GetGoals()->operator[](
                    i))->GetPolygon2D()->GetVertices();
            for (auto &v: *vertices)
                out << v << " ";
        } else {
