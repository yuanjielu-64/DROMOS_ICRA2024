...
namespace Antipatrea {
    ...
    void GManagerMP::OnSolve(void) {
        ...
        if (GetSetup()->GetMP() && GetSetup()->GetMP()->IsSolved() == false) {
            Timer::Start(clk);
            if (m_canBeSolved) {

                //    GetSetup()->GetMP()->Solve(1000, 7.0, m_canBeSolved);
                GetSetup()->GetMP()->Solve(1000, 7.0, m_canBeSolved);
                ...
                Logger::m_out << "[runtimeForPRM                = " << Stats::GetSingleton()->GetValue("TimeForPRM")<< std::endl
                              << "[runtimeForSolve              = " << Stats::GetSingleton()->GetValue("TimeSolve") << "]" << std::endl
                              << "[solved                       = " << m_solution.GetStateSequence()->size() << "]" << std::endl
                              << "[distance                     = " << m_solution.GetCost() << "]" << std::endl
