...
namespace Antipatrea {
    ...
    void DecompositionPRM::Construct(const double tmax) {
        Timer::Clock clk;
        Timer::Clock tRegion;
        Timer::Clock tConnectRegion;
        Timer::Clock totalPRM;
        Timer::Start(clk);
        Timer::Start(totalPRM);
        ...
        while (!AreInitialAndGoalsConnected() && GetGraph()->GetNrVertices() < GetMaxNrVertices()) {
            ...
            Stats::GetSingleton()->AddValue("TimeAddRegions", Timer::Elapsed(tRegion));
            
            Timer::Start(tConnectRegion);
            ConnectRegions(tmax - Timer::Elapsed(clk));
            Stats::GetSingleton()->AddValue("TimeConnectRegions", Timer::Elapsed(tConnectRegion));
            Stats::GetSingleton()->AddValue("NrRegions", 1);

        }

        auto a = Stats::GetSingleton()->GetValue("TimeAddRegions");
        auto b = Stats::GetSingleton()->GetValue("TimeConnectRegions");
        ...
        for (auto &iter: *edges) {
            ...
            if (GetGraph()->AreVerticesPathConnected(idFrom, key) == false ||
                GetGraph()->AreVerticesPathConnected(idTo, key) == false)
        ...
        Stats::GetSingleton()->AddValue("TimeConnectRegions", Timer::Elapsed(tConnectRegion));
        Stats::GetSingleton()->AddValue("TimeForPRM", Timer::Elapsed(totalPRM));
