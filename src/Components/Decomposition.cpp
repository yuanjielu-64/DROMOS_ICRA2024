...
namespace Antipatrea {
    ...
    bool Decomposition::AreInitialAndGoalsConnected(void) {
        ...
        for (int i = 0; i < (int) m_pairsInitialAndGoalsUnconnected.size();)
            if (graph->AreVerticesPathConnected(m_pairsInitialAndGoalsUnconnected[i],
                ...
                i += 2;
        return m_pairsInitialAndGoalsUnconnected.size() == 0;
    }

    void Decomposition::Predictions(void) {
        if (m_predictionsComputed)
            return;
        m_predictionsComputed = true;
        ...
        std::vector<double> resultsRuntimes;
        std::vector<double> resultsDistances;

        for (int i = 0; i < nr; ++i) {
            auto r = dynamic_cast<Region *>(m_graph.GetVertexByIndex(i));
            for (int j = 0; j < ng; ++j) {
                auto g = (*goals)[j];
                positions.push_back(CleanValue(r->GetCfg()[0]));
                positions.push_back(CleanValue(r->GetCfg()[1]));
                positions.push_back(CleanValue(g->GetCfg()[0]));
                positions.push_back(CleanValue(g->GetCfg()[1]));
                pairs.push_back(i);
                pairs.push_back(j);
            }
        }
        resultsRuntimes.resize(positions.size() / 4);
        resultsDistances.resize(positions.size() / 4);

        Timer::Clock clk;
        Timer::Start(clk);

        Logger::m_out << "...making distance predictions for " << resultsDistances.size() << " pairs " << std::endl;
        Predict(GetSimulator()->m_modelPredictDistance, resultsDistances.size(), &positions[0], &resultsDistances[0]);
        Logger::m_out << "...done with distance predictions" << std::endl;

        Logger::m_out << "...making runtime predictions for " << resultsRuntimes.size() << " pairs " << std::endl;
        Predict(GetSimulator()->m_modelPredictRuntime, resultsRuntimes.size(), &positions[0], &resultsRuntimes[0]);
        Logger::m_out << "...done with runtime predictions" << std::endl;


        Logger::m_out << "...take time =  " << Timer::Elapsed(clk)  << std::endl;
        Logger::m_out << "...storing the predicted results" << std::endl;
        int where = 0;
        for (int i = 0; i < nr; ++i) {
            auto r = dynamic_cast<Region *>(m_graph.GetVertexByIndex(i));
            r->m_predictedTimesToGoals.resize(ng);
            r->m_predictedDistancesToGoals.resize(ng);
            r->m_predictedCombinedToGoals.resize(ng);
            for (int j = 0; j < ng; ++j) {
                r->m_predictedTimesToGoals[j] = resultsRuntimes[where];
                r->m_predictedDistancesToGoals[j] = resultsDistances[where];
                r->m_predictedCombinedToGoals[j] = 0.1 * resultsRuntimes[where] + 0.9 * resultsDistances[where];
                ++where;
            }
        }
    }
    ...
    void Decomposition::PathsToGoals(void) {
        PathsToGoalsRadix();
        return;
        ...
        gsi.SetGraph(GetGraph());
        gs.SetGraphAbstraction(&gsi);

        m_maxPathCosts.resize(ng);
        for (int i = 0; i < ng; ++i) {
            m_maxPathCosts[i] = 0;
            gs.AStar((*goals)[i]->GetKey(), breakEarly, gid);
            for (int j = 0; j < nv; ++j) {
                r = dynamic_cast<Region *>(GetGraph()->GetVertexByIndex(j));
                if (r != NULL) {
                    if (r->GetPathDataToGoals()->size() < ng) {
                        ...
                        data = (*(r->GetPathDataToGoals()))[j];
                    data->m_cost = gs.GetPathCostFromStart(r->GetKey());
                    data->m_path.clear();
                    gs.GetReversePathFromStart(r->GetKey(), data->m_path);
                    data->m_pts.clear();
                    PathFromIdsToPoints(data->m_path.size(), &(data->m_path[0]), data->m_pts);
            ...
            Logger::m_out << "..processed goal " << i << std::endl;
        }

        Stats::GetSingleton()->AddValue("TimePathsToGoals", Timer::Elapsed(clk));
    }

    void Decomposition::AddGoals(void) {
        RegionGeometric *r;
        auto goals = GetProblem()->GetGoals();
        const int ng = goals->size();
